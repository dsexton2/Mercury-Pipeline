...
class FastQBuilder
  ...
  def writeFinalSequencesPE()
     ...
     puts "Read 1 files "
     @read1FileList.each do |name|
      puts name.to_s
     end

     puts "Read 2 files"
     @read2FileList.each do |name|
       puts name.to_s
     end

     idx = 0

     puts "Number of files : " + @read1FileList.size.to_s
     while(idx < @read1FileList.size)
       puts "Value of index = " + idx.to_s

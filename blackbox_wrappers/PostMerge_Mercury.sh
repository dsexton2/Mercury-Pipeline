...
do
	...
	DIR_BAM=`ls $DIR_BAM_IN/*.bam`   ### Comment this back in if the path does not contain the bam ###
...
echo "samtools index ${BAM}" | msub -q normal -d ${DIR}  -V -l nodes=1:ppn=1,mem=8000mb -N ${FC}_index2
sleep 3
### Here is the command for generating realignment
echo "java -Xmx26000M -jar /stornext/snfs5/next-gen/Illumina/ipipeV2/gatk/GenomeAnalysisTK-1.3-8-gb0e6afe/GenomeAnalysisTK.jar -T RealignerTargetCreator -I ${BAM} -R $REF -o ${FC}.intervals" | msub -q normal -V -d $DIR -l depend=afterok:${FC}_index2 -l nodes=1:ppn=8,mem=28G -N ${FC}_intervals
sleep 3
echo "java -Xmx40000M -jar /stornext/snfs5/next-gen/Illumina/ipipeV2/gatk/GenomeAnalysisTK-1.3-8-gb0e6afe/GenomeAnalysisTK.jar -T IndelRealigner -I ${BAM} -R $REF -targetIntervals ${FC}.intervals -o ${FC}_realigned.bam -compress 1" | msub -q normal -V -d $DIR -l depend=afterok:${FC}_intervals -l nodes=1:ppn=8,mem=44G -N ${FC}_realign
sleep 3
### Here is the command which generates the indexed BAM
echo "samtools index ${FC}_realigned.bam" | msub -q normal -d ${DIR}  -V -l depend=afterok:${FC}_realign -l nodes=1:ppn=1,mem=8000mb -N ${FC}_index3
sleep 3
### Here is the command to generate pileup
echo "/stornext/snfs5/next-gen/software/samtools-0.1.7/samtools pileup -vcf ${REF} ${FC}_realigned.bam > ${FC}.pileup" | msub -V -q normal -d ${DIR} -l depend=afterok:${FC}_index3 -l nodes=1:ppn=1,mem=8000mb -N ${FC}_pileup 
sleep 3

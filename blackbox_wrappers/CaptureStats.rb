...
# TODO: Add functionality to upload results to LIMS
class CaptureStats
  ...
  # Method to generate path names for various supporting scripts used to
  # calculate the capture stats.
  def buildPathNames()
    yamlConfigFile = PathInfo::CONFIG_DIR + "/config_params.yml" 

    configReader = YAML.load_file(yamlConfigFile)

    @captureCodeDir = configReader["captureStats"]["codeDirectory"]
    @codeFile       = configReader["captureStats"]["captureCode"]

    if @captureCodeDir == nil || @captureCodeDir.empty?()
      raise "Error: Did not find directory path of capture stats program in config file"
    end

    if @codeFile == nil || @codeFile.empty?()
       raise "Error: Did not find the name of the program to execute to generate capture stats"
    end

    @codeFile = @captureCodeDir + "/" + @codeFile

    @outputDir      = "capture_stats"
    @resultPrefix   = "cap_stats"
    @limsScript     = PathInfo::LIMS_API_DIR + "/setIlluminaCaptureResults.pl"
  end


  # Get flowcell barcode from the config file
  def getFlowcellBarcode()
     # Obtain flowcell barcode to identify this sequencing event for uploading
     # results to LIMS or send email
     inputParams = BWAParams.new()
     inputParams.loadFromFile()
     @fcBarcode  = inputParams.getFCBarcode()

     if @fcBarcode == nil || @fcBarcode.empty?()
       raise "Did not find flowcell barcode"
     end
  end
